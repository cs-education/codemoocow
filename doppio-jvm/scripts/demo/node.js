...
(function() {
  "use strict";
  var CacheSource, CompositedFileSource, DoppioFile, FSState, FileIndex, FileSource, GetIEByteArray_ByteStr, LocalStorageSource, Stat, WebserverSource, basename, canUsePostMessage, config, fs_state, handleMessage, inject_vbscript, messageName, root, setZeroTimeout, timeouts, win, _, _ref, _ref1, _ref2, _ref3, _ref4,
  ...
  config = (_ref = window.nodeconfig) != null ? _ref : window.nodeconfig = {};

  if ((_ref1 = config.home) == null) {
    config.home = '/home/doppio';
  }

  if ((_ref2 = config.base_url) == null) {
    config.base_url = '';
  }

  if ((_ref3 = config.listing_file) == null) {
    config.listing_file = '/browser/listings.json';
  }

  if ((_ref4 = config.storage_prefix) == null) {
  ...
  FileIndex = (function() {
    ...
    FileIndex.prototype._mkdir = function(components) {
      var c, cur_dir, _i, _len, _ref5;

      cur_dir = this.index;
      for (_i = 0, _len = components.length; _i < _len; _i++) {
        c = components[_i];
        if ((_ref5 = cur_dir[c]) == null) {
          cur_dir[c] = {};
        }
        cur_dir = cur_dir[c];
      }
  ...
  CompositedFileSource = (function(_super) {
    ...
    CompositedFileSource.prototype._get_applicable_sources = function(path) {
      var a_mnt_pt, applicable, _i, _len, _ref5;
  ...
  LocalStorageSource = (function(_super) {
    ...
    function LocalStorageSource(mnt_pt, prefix) {
      var i, key, _i, _ref5;

      this.prefix = prefix != null ? prefix : '';
      LocalStorageSource.__super__.constructor.call(this, mnt_pt);
      this.index = new FileIndex();
      for (i = _i = 0, _ref5 = localStorage.length; 0 <= _ref5 ? _i < _ref5 : _i > _ref5; i = 0 <= _ref5 ? ++_i : --_i) {
        key = localStorage.key(i);
        if (key.indexOf(this.prefix) === 0) {
          this.index.add_file(key.substring(this.prefix.length), null);
        }
      }
    }
    ...
    LocalStorageSource.prototype.fetch = function(path) {
      ...
      data = localStorage.getItem(this.prefix + path);
      if (data != null) {
    ...
    LocalStorageSource.prototype.store = function(path, file) {
      ...
      try {
        if (file.mod && !file.temp) {
          localStorage.setItem(this.prefix + path, file.toJSON());
        }
    ...
    LocalStorageSource.prototype.rm = function(path) {
      ...
      if (typeof listing !== 'boolean') {
        for (_i = 0, _len = listing.length; _i < _len; _i++) {
          item = listing[_i];
          itPath = path + '/' + item;
          localStorage.removeItem(this.prefix + itPath);
        }
      } else if (localStorage.getItem(this.prefix + path) != null) {
  ...
  WebserverSource = (function(_super) {
    ...
    WebserverSource.prototype._download_file = function(path) {
      var array, char, data, i, req, typed_array, url_path, _i, _len;

      if ((this.index != null) && this.index.get_file(this.mnt_pt + path) === false) {
        return null;
      }
      data = null;
      url_path = this.url_base + path;
      if (!$.browser.msie) {
        $.ajax(url_path, {
          type: 'GET',
          dataType: 'text',
    ...
    function WebserverSource(mnt_pt, url_base, listings_path) {
      var idx_data;

      this.url_base = url_base;
      WebserverSource.__super__.constructor.call(this, mnt_pt);
      this.url_base.replace(/\/$/, '');
  ...
  FSState = (function() {
    function FSState(home, base_url, listing_file, storage_prefix) {
      var f, mainSource;

      this.home = home;
      this.pwd = this.home;
      mainSource = new CompositedFileSource('/', [new LocalStorageSource('/', storage_prefix), new WebserverSource(this.home, base_url, listing_file)]);
      this.files = new CacheSource('/', mainSource);
      f = new DoppioFile(this.home + '/Hello.txt', "Welcome to Doppio!");
      f.mod = true;
      this.files.store(f.path, f);
    }

    FSState.prototype.resolve = function(path) {
      var absolute, c, components, i, idx, processed, pwdCmp, _i, _j, _k, _len, _len1, _ref5;
      ...
      if (!absolute) {
        _ref5 = this.pwd.split('/');
        for (_j = _ref5.length - 1; _j >= 0; _j += -1) {
          pwdCmp = _ref5[_j];
    ...
    writeSync: function(fd, buffer, offset, len, pos) {
      ...
      if (buffer.readUInt8 != null) {
        str = ((function() {
          ...
          _results = [];
          for (i = _i = offset, _ref5 = offset + len; _i < _ref5; i = _i += 1) {
