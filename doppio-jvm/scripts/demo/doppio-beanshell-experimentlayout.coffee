...
class window.CodeRunner
    ...
    constructor : (@element) ->
        ...
        @runJavaBtn = $('<button/>',{name:'runjava-ctrl-run',text:'run',type:'button'})
        @stopJavaBtn = $('<button/>',{name:'runjava-ctrl-stop',text:'stop',type:'button'})
        @resetCodeBtn = $('<button/>',{name:'runjava-ctrl-reset',text:'reset',type:'button'})
        ...
        @resizeEditor= => 
            ...
            #@editorDiv.css(@blurb )
            @editorDiv.width =200 + position.right - position.left
            @editorDiv.height=200 + position.bottom - position.top
