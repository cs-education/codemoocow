...
(function() {
  ...
  root.loadJSONConfigs = function() {
    var configFail, exception, game, readGameData, _i, _len, _ref;
    ...
    configFail = false;
    try {
      root.readJSON('config/defaults.json', function(data) {
        return root.gameDefaults = data;
      });
      root.readJSON('config/quest1.json', function(data) {
        return root.quest = data;
      });
      readGameData = function(gameData) {
        if (gameData) {
          root.addToObject(root.gameDefaults, gameData);
          root.convertShorthandToCode(gameData);
          root.addHintsToCode(gameData);
          return root.gameDescriptions[game] = gameData;
        }
      };
      _ref = root.quest.games;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        game = _ref[_i];
        root.readJSON("config/" + game + ".json", readGameData);
      }
      root.readJSON('config/visualMaster.json', function(data) {
        return root.visualMaster = data;
      });
    } catch (_error) {
      exception = _error;
      configFail = true;
